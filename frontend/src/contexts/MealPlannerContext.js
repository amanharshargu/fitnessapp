...
export function MealPlannerProvider({ children }) {
  ...
  const searchRecipesWithIngredients = useCallback(async (ingredients) => {
    try {
      ...
      const searchPromises = [
        ...
        // Individual searches
        ...ingredients.map((ingredient) => axios.get('https://api.edamam.com/api/recipes/v2', {
          params: {
            type: 'public',
            q: ingredient.name,
            app_id: EDAMAM_APP_ID,
      ...
      // Shuffle the recipes
      for (let i = allRecipes.length - 1; i > 0; i -= 1) {
  ...
  const fetchRecipeDetails = useCallback(async (mealPlanData) => {
    try {
      ...
      const recipePromises = mealPlanData.selection
        .flatMap((day) => Object.values(day.sections).map((meal) => {
